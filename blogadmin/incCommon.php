	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'featured_homestay' => array('Editors choice', '', 'resources/table_icons/bullet_star.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'featured_homestay' => "`featured_homestay`.`id` as 'id', IF(    CHAR_LENGTH(`blogs1`.`title`) || CHAR_LENGTH(`blogs1`.`author`), CONCAT_WS('',   `blogs1`.`title`, ' :author ', `blogs1`.`author`), '') as 'blog'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'featured_homestay' => "`featured_homestay` LEFT JOIN `blogs` as blogs1 ON `blogs1`.`id`=`featured_homestay`.`blog` "
		);
