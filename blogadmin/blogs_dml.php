...
function blogs_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	// child table: featured_homestay
	$res = sql("select `id` from `blogs` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `featured_homestay` where `blog`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "featured_homestay", $RetMsg);
		return $RetMsg;
